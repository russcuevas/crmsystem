...
@section('content')
                            ...
                            <tr>
                                ...
                                <td style="text-align: center;">
                                    <div style="display: flex; gap: 0.35rem; justify-content: center; align-items: center;">
                                        @if (!$sy->is_active)
                                            <form action="{{ route('admin.school_years.setActive', $sy->id) }}" method="POST" style="display: inline;">
                                                @csrf
                                        ...
                                        @if (!$sy->is_active)
                                            <form action="{{ route('admin.school_years.destroy', $sy->id) }}" method="POST" style="display: inline;" onsubmit="return confirm('Are you sure you want to delete S.Y. {{ $sy->school_year }}?');">
                                                @csrf
                                                @method('DELETE')
    ...
    <div class="modal-overlay" id="addSchoolYearModal">
        <div class="modal-card">
            ...
            <form action="{{ route('admin.school_years.store') }}" method="POST">
                @csrf
...
@push('scripts')
    ...
    <script>
        ...
        function openEditSchoolYearModal(sy) {
            ...
            form.action = "{{ url('/admin/school-years/update') }}/" + sy.id;
