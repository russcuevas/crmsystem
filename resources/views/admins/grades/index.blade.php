                        ...
                        @forelse($categories as $cat)
                            ...
                            <div class="modal-overlay" id="editCategoryModal_{{ $cat->id }}">
                                <div class="modal-card">
                                    ...
                                    <form action="{{ route('admin.grades.category.update', $cat->id) }}" method="POST">
                                        @csrf
                                        <div class="modal-body">
                                            <div class="form-group">
                                                <label>Academic Period</label>
...
@push('scripts')
    ...
    <script>
        ...
        function resetTotalGradesAjax() {
            if (!confirm('Are you sure you want to reset and delete all computed grades for {{ $targetSecSubName }}?')) {
                return;
            }

            const btn = document.getElementById('btnResetTotalGrades');
            ...
            fetch('{{ route('admin.grades.reset.total') }}', {
                ...
                .then(data => {
                    if (btn) btn.disabled = false;
                    if (spinner) spinner.style.display = 'none';
                    if (btnText) btnText.textContent = '🗑️ Reset Grades';

                    if (data.status === 'success') {
                        const rows = document.querySelectorAll('tr[data-breakdown-row]');
                        rows.forEach(row => {
                            row.querySelectorAll('.period-val').forEach(el => el.textContent = '-');
                        ...
                        alert('⚠️ Error resetting grades: ' + (data.message || 'Unknown error'));
                    }
                })
                .catch(error => {
                    if (btn) btn.disabled = false;
                    ...
                    alert('⚠️ An error occurred while resetting total grades.');
                });
        }
    </script>
                ...
                <div class="modal-body" style="max-height: 78vh; overflow-y: auto; padding: 1.5rem;">
                    <div
                        ...
                        <div style="display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap;">
                            ...
                            <button type="button" id="btnResetTotalGrades" onclick="resetTotalGradesAjax()"
                                class="btn-danger"
                                style="background: #dc2626; color: #ffffff; font-weight: 800; padding: 0.6rem 1.2rem; border-radius: 10px; display: inline-flex; align-items: center; gap: 0.5rem; cursor: pointer; box-shadow: 0 4px 6px -1px rgba(220, 38, 38, 0.3); border: none; transition: all 0.2s ease;"
                                title="Delete computed grades for this subject">
                                <span id="resetBtnSpinner" style="display: none;">⌛</span>
                                <span id="resetBtnText">🗑️ Reset Grades</span>
                            </button>
                        </div>
                    </div>
