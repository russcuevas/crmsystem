...
@section('content')
    ...
    <div
        ...
        <div>
            ...
            <div style="display: flex; gap: 0.5rem; margin-top: 0.5rem; flex-wrap: wrap;">
                <span
                    style="background: #fee2e2; color: #991b1b; padding: 0.25rem 0.65rem; border-radius: 0.375rem; font-size: 0.775rem; font-weight: 800;">
                    Student ID: {{ $student->student_number }}
                </span>
                @if ($student->lrn)
                    <span
                        style="background: #fef3c7; color: #92400e; padding: 0.25rem 0.65rem; border-radius: 0.375rem; font-size: 0.775rem; font-weight: 800;">
                        LRN: {{ $student->lrn }}
                    </span>
                @endif
                <span
                    style="background: #d1fae5; color: #065f46; padding: 0.25rem 0.65rem; border-radius: 0.375rem; font-size: 0.775rem; font-weight: 800;">
                    {{ $student->gradeLevel ? $student->gradeLevel->name : 'N/A' }}
                </span>
                @if ($student->course)
                    <span
                        style="background: #e0e7ff; color: #3730a3; padding: 0.25rem 0.65rem; border-radius: 0.375rem; font-size: 0.775rem; font-weight: 800;">
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 1.5rem;">
        <div
            ...
            <h3
                style="font-size: 1.05rem; font-weight: 800; color: #0f172a; margin-bottom: 1.25rem; border-bottom: 1px solid #e2e8f0; padding-bottom: 0.75rem;">
                Personal Details
            </h3>
            <div style="display: flex; flex-direction: column; gap: 0.85rem; font-size: 0.875rem;">
                <div style="display: flex; justify-content: space-between;">
                    ...
                    <span
                        style="font-weight: 700; color: #0f172a;">{{ $student->user ? $student->user->email : 'N/A' }}</span>
                </div>
                <div style="display: flex; justify-content: space-between;">
                ...
                <div style="display: flex; justify-content: space-between;">
                    ...
                    <span
                        style="font-weight: 700; color: #0f172a;">{{ $student->birthday ? \Carbon\Carbon::parse($student->birthday)->format('F d, Y') : 'N/A' }}</span>
                </div>
                <div style="display: flex; justify-content: space-between;">
            ...
            @if ($student->enrollments->isNotEmpty())
                <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                    @foreach ($student->enrollments as $enr)
                        <div
                            style="padding: 0.75rem 1rem; background: #f8fafc; border: 1px solid #f1f5f9; border-radius: 0.5rem; display: flex; justify-content: space-between; align-items: center;">
                            <div>
                            ...
                            <span
                                style="background: #d1fae5; color: #065f46; padding: 0.2rem 0.5rem; border-radius: 4px; font-size: 0.75rem; font-weight: 700; text-transform: uppercase;">
                                {{ $enr->status }}
                            </span>
                        </div>
                    @endforeach
