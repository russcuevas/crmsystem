...
@section('title', 'GNHS-P - SHS Class Record & Grading')
...
@section('content')
    ...
    <!-- Header Controls: Semester, Subject Selector & Academic Periods (Prelim, Midterm, Finals) -->
    <div
        ...
        <div style="display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 1rem;">
            ...
            <form action="{{ route('senior_high_school.grades.page') }}" method="GET"
                ...
                <div style="display: flex; align-items: center; gap: 0.4rem;">
                    <label style="font-weight: 700; font-size: 0.875rem; color: #0f172a; white-space: nowrap;">
                        <i class="fa-solid fa-calendar-days" style="color: #4f46e5;"></i> Semester:
                    </label>
                    <select name="semester" onchange="this.form.submit()"
                        style="padding: 0.55rem 0.85rem; border: 1.5px solid #cbd5e1; border-radius: 8px; font-size: 0.875rem; font-weight: 700; color: #1e1b4b; background: #eef2ff; outline: none; cursor: pointer;">
                ...
                <div style="display: flex; align-items: center; gap: 0.4rem; flex: 1;">
                    <label style="font-weight: 700; font-size: 0.875rem; color: #0f172a; white-space: nowrap;">
                        <i class="fa-solid fa-book-open" style="color: #4f46e5;"></i> Subject:
                    </label>
                        ...
                        @else
                            @foreach ($handledSubjects as $hs)
                                <option value="{{ $hs->id }}"
                                    ...
                                    {{ $hs->subject ? $hs->subject->subject_name ?? $hs->subject->name : 'N/A' }} - Section:
                                    {{ $hs->classSection ? $hs->classSection->section_name : 'N/A' }}
                                    ({{ $hs->classSection && $hs->classSection->gradeLevel ? $hs->classSection->gradeLevel->name : '' }}
                                    {{ $hs->classSection && $hs->classSection->course ? '[' . $hs->classSection->course->course_code . ']' : '' }})
                                </option>
                            @endforeach
                        @endif
                    </select>
    ...
    @if (isset($isParentSubject) && $isParentSubject)
        <!-- Parent Subject Component Tabs Bar -->
        <div
            ...
            <div style="display: flex; flex-wrap: wrap; gap: 0.5rem; align-items: center;">
                @foreach ($subSectionSubjects as $subSec)
                    ...
                    @endphp
                    <a href="{{ route('senior_high_school.grades.page', ['semester' => $selectedSemester, 'section_subject_id' => $currentSectionSubject->id, 'academic_period' => $selectedPeriod, 'sub_subject_id' => $subSec->id]) }}"
                        ...
                        <i class="fa-solid fa-book"></i> {{ $sName }}
                    </a>
                @endforeach

                <a href="{{ route('senior_high_school.grades.page', ['semester' => $selectedSemester, 'section_subject_id' => $currentSectionSubject->id, 'academic_period' => $selectedPeriod, 'sub_subject_id' => 'summary']) }}"
            ...
            $displaySubjName =
                isset($isParentSubject) && $isParentSubject && is_object($activeSubSectionSubject)
                    ...
                    : ($currentSectionSubject->subject
                        ? $currentSectionSubject->subject->subject_name ?? $currentSectionSubject->subject->name
                        : 'N/A');
        @endphp
        @if ($activeSubSectionSubject !== 'summary')
            ...
            <div
                ...
                <div>
                    ...
                    <div style="color: #64748b; font-weight: 600; font-size: 0.875rem; margin-top: 0.2rem;">
                        Section:
                        <strong>{{ $currentSectionSubject->classSection ? $currentSectionSubject->classSection->section_name : 'N/A' }}</strong>
                        @if ($currentSectionSubject->classSection && $currentSectionSubject->classSection->gradeLevel)
                            ({{ $currentSectionSubject->classSection->gradeLevel->name }})
                        @endif
                        ...
                        @endif
                        | Semester: <strong style="color: #4f46e5;">{{ $selectedSemester }}</strong>
                ...
                <div style="display: flex; flex-wrap: wrap; gap: 0.5rem; align-items: center;">
                    ...
                    <form action="{{ route('senior_high_school.grades.compute.total') }}" method="POST"
                        ...
                        <input type="hidden" name="academic_period" value="{{ $selectedPeriod }}">
                        <button type="submit"
                            style="background: #10b981; color: #ffffff; padding: 0.5rem 0.9rem; border-radius: 0.5rem; font-weight: 700; font-size: 0.8rem; border: none; cursor: pointer; display: flex; align-items: center; gap: 0.4rem; box-shadow: 0 2px 6px rgba(16, 185, 129, 0.3);">
            ...
            <!-- Class Record Spreadsheet Table Card -->
                ...
                @if ($enrolledStudents->isNotEmpty())
                    <table class="grading-table">
                        <thead>
                            <tr>
                                <th rowspan="2"
                                    style="min-width: 200px; text-align: left; position: sticky; left: 0; background: #f8fafc; z-index: 5;">
                                    Student Name</th>
                                @foreach ($categories as $cat)
                                    <th colspan="{{ $cat->gradingTasks->isNotEmpty() ? $cat->gradingTasks->count() + 1 : 1 }}"
                                        ...
                                        <div
                                            ...
                                            <span
                                                style="font-weight: 800; color: #3730a3;">{{ $cat->category_name ?? $cat->name }}
                                            ...
                                            <form
                                                action="{{ route('senior_high_school.grades.category.destroy', $cat->id) }}"
                                                method="POST" style="display: inline;"
                                                onsubmit="return confirm('Are you sure you want to delete category \'{{ $cat->category_name ?? $cat->name }}\' and all its tasks?');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
                                ...
                                <th rowspan="2"
                                    style="text-align: center; background: #ecfdf5; color: #065f46; min-width: 110px;">
                                    {{ $selectedPeriod }} Grade</th>
                                <th rowspan="2"
                                    style="text-align: center; background: #ecfdf5; color: #065f46; min-width: 90px;">
                                    Remarks</th>
                            </tr>
                            <tr>
                                @foreach ($categories as $cat)
                                        ...
                                        @foreach ($cat->gradingTasks as $t)
                                            <th style="text-align: center; font-size: 0.775rem;">
                                                <div
                                                    ...
                                                    <form
                                                        action="{{ route('senior_high_school.grades.task.destroy', $t->id) }}"
                                                        method="POST" style="display: inline;"
                                    ...
                                    @foreach ($attendanceDates as $adate)
                                        <th style="text-align: center; font-size: 0.775rem; background: #fff7ed;">
                                            <div
                                                ...
                                                <form
                                                    action="{{ route('senior_high_school.grades.attendance.date.destroy') }}"
                                                    method="POST" style="display: inline;"
                        ...
                        <tbody>
                            @foreach ($enrolledStudents as $stu)
                                <tr>
                                    <td
                                        style="font-weight: 700; color: #0f172a; position: sticky; left: 0; background: #ffffff; z-index: 5;">
                                    ...
                                    @if ($attendanceDates->isNotEmpty())
                                        @foreach ($attendanceDates as $adate)
                                            ...
                                            <td style="text-align: center; background: #fffdfb;">
                                                <select
                                                    onchange="saveAttendance({{ $stu->id }}, {{ $targetSecSubId }}, '{{ $formattedDate }}', this.value, this)"
                                    ...
                                    @php
                                        ...
                                        if ($hasAnyTaskScores) {
                                            ...
                                            $finalGradeVal = number_format(
                                                $computedGrades[$stu->id]->final_quarter_grade,
                                                2,
                                            );
    ...
    @else
        <div
            ...
            <p style="font-size: 1rem; font-weight: 700;">No handled teaching subject selected for
                {{ $selectedSemester }}.</p>
        </div>
    @endif
    ...
    <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(15,23,42,0.6); backdrop-filter: blur(4px); display: none; align-items: center; justify-content: center; z-index: 9999;"
        ...
        <div
            ...
            <div
                ...
                📌 Component: <strong>{{ $displaySubjName ?? 'Subject' }}</strong> ({{ $selectedPeriod }})
            </div>
            <form action="{{ route('senior_high_school.grades.category.store') }}" method="POST">
                @csrf
                <input type="hidden" name="class_section_subject_id" value="{{ $targetSecSubId ?? '' }}">
                <input type="hidden" name="academic_period" value="{{ $selectedPeriod }}">
    ...
    <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(15,23,42,0.6); backdrop-filter: blur(4px); display: none; align-items: center; justify-content: center; z-index: 9999;"
        ...
        <div
            ...
            <form action="{{ route('senior_high_school.grades.task.store') }}" method="POST">
                @csrf
                <div style="margin-bottom: 1rem;">
                    ...
                    <select name="grading_category_id" required
                        style="width: 100%; padding: 0.5rem; border: 1px solid #cbd5e1; border-radius: 6px;">
                        @foreach ($categories as $cat)
                            <option value="{{ $cat->id }}">{{ $cat->category_name ?? $cat->name }}
                                ({{ floatval($cat->weight) }}%)</option>
                        @endforeach
                    </select>
                </div>
                <div style="margin-bottom: 1rem;">
                ...
                <div style="margin-bottom: 1.25rem;">
                    ...
                    <input type="number" name="max_score" required min="1" step="0.01"
                        style="width: 100%; padding: 0.5rem; border: 1px solid #cbd5e1; border-radius: 6px;">
                </div>
                <div style="display: flex; justify-content: flex-end; gap: 0.5rem;">
    ...
    <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(15,23,42,0.6); backdrop-filter: blur(4px); display: none; align-items: center; justify-content: center; z-index: 9999;"
        ...
        <div
            ...
            <form action="{{ route('senior_high_school.grades.attendance.date.store') }}" method="POST">
                @csrf
                <input type="hidden" name="class_section_subject_id" value="{{ $targetSecSubId ?? '' }}">
                <input type="hidden" name="academic_period" value="{{ $selectedPeriod }}">
                <div style="margin-bottom: 1.25rem;">
        ...
        <div
            ...
            <form id="editCatForm" method="POST">
                ...
                <div style="margin-bottom: 1rem;">
                    ...
                    <input type="text" id="edit_cat_name" name="category_name" required
                        style="width: 100%; padding: 0.5rem; border: 1px solid #cbd5e1; border-radius: 6px;">
                </div>
                <div style="margin-bottom: 1.25rem;">
                    <label style="font-size: 0.8rem; font-weight: 700; color: #475569;">Weight Percentage (%)</label>
    ...
    <div style="position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(15,23,42,0.6); backdrop-filter: blur(4px); display: none; align-items: center; justify-content: center; z-index: 9999;"
        ...
        <div
            ...
            <form id="editTaskForm" method="POST">
                ...
                <div style="margin-bottom: 1.25rem;">
                    <label style="font-size: 0.8rem; font-weight: 700; color: #475569;">Max Score</label>
                    <input type="number" id="edit_max_score" name="max_score" required min="1" step="0.01"
                        style="width: 100%; padding: 0.5rem; border: 1px solid #cbd5e1; border-radius: 6px;">
                </div>
                <div style="display: flex; justify-content: flex-end; gap: 0.5rem;">
...
@push('scripts')
    ...
    <script>
        ...
        function openEditCategoryModal(cat) {
            $('#editCatForm').attr('action', "/senior-high-school/grades/category/update/" + cat.id);
        ...
        function openEditTaskModal(task) {
            $('#editTaskForm').attr('action', "/senior-high-school/grades/task/update/" + task.id);
        ...
        function saveScore(studentId, taskId, score, inputElem) {
            $.ajax({
                url: "{{ route('senior_high_school.grades.update_score') }}",
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
        ...
        function saveAttendance(studentId, sectionSubjId, date, status, selectElem) {
            $.ajax({
                url: "{{ route('senior_high_school.grades.attendance.update_status') }}",
                method: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    student_id: studentId,
